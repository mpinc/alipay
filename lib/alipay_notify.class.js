...
function AlipayNotify(alipay_config,logger){
    /**
     * HTTPS形式消息验证地址
     */
    ...
    if(Object.keys(_POST).length == 0) {//判断POST来的数组是否为空
        ...
        /*if (null != _POST["notify_id"]) {
            this.getResponse(_POST["notify_id"], function(responseTxt){
                this.logger.debug("ali response text:"+respnseTxt);
            	callback(responseTxt == 'true' && isSign);
            });
        }
        else{*/
        this.logger.debug("callback:"+(responseTxt == 'true' && isSign));
        return callback(responseTxt == 'true' && isSign);
       // }
    }
}
...
AlipayNotify.prototype.verifyReturn = function(_GET, callback){
    if(Object.keys(_GET).length == 0) {//判断POST来的数组是否为空
        ...
        //comment out the remote check  for now
        //获取支付宝远程服务器ATN结果（验证是否是支付宝发来的消息）
        var responseTxt = 'true';
        //验证
        //responsetTxt的结果不是true，与服务器设置问题、合作身份者ID、notify_id一分钟失效有关
...
AlipayNotify.prototype.getSignVeryfy = function(para_temp, sign){
    ...
    this.logger.debug("prestr :"+prestr);
    ...
    if(sign_type == "MD5"){
        this.logger.debug("key :"+this.alipay_config['key']);
        this.logger.debug("sign :"+sign);
        generatedSign=md5_f.getMd5Verify(prestr,this.alipay_config['key']);
        this.logger.debug("generated sign:"+generatedSign);
        isSign = (sign==generatedSign);
        this.logger.debug("ISSIGN:"+isSign);
    }
    else{
        isSign = false;
    }
