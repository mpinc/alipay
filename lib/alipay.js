...
function Alipay(alipay_config,logger){
    ...
    if (logger !=null){
        ...
        this.logger=createLogger("alipay.js");
    }
}
...
Alipay.prototype.create_direct_pay_by_user = function(data, res){
	var html_text = this.create_direct_pay_by_user_form(data);
	res.send(html_text);
}
...
//it generate the html form only

Alipay.prototype.create_direct_pay_by_user_form = function(data){
    ...
    this.logger.debug("alipay request form: "+html_text);
    return html_text;
}
...
Alipay.prototype.trade_create_by_buyer_notify = function(req, res,callback){
	var self = this;

	var _POST = req.body;
	//计算得出通知验证结果
	var alipayNotify = new AlipayNotify(this.alipay_config,this.logger);
	//验证消息是否是支付宝发出的合法消息
	alipayNotify.verifyNotify(_POST, function(verify_result){
		if(verify_result) {//验证成功
            ...
            this.logger("verify_succeed");
			res.send("success");
            callback(null,_POST);
		}
		else{
			//验证失败
            this.logger("verify_fail");
			self.emit("verify_fail");
			res.send("fail");
            callback(new Error("alipay verify failed"),null);
		}
	});	
}
...
Alipay.prototype.verify_direct_pay_by_user_notify = function(_POST,callback){
	var self = this;
    //商户订单号
    var out_trade_no = _POST['out_trade_no'];
    self.logger.debug("out_trade_no :"+out_trade_no);
    //支付宝交易号
    var trade_no = _POST['trade_no'];
    self.logger.debug("trade_no :"+trade_no);
    //交易状态
    var trade_status = _POST['trade_status'];
    self.logger.debug("trade_status :"+trade_status);
	...
	//验证消息是否是支付宝发出的合法消息
	alipayNotify.verifyNotify(_POST, function(verify_result){
        self.logger.debug("verify_result:"+verify_result);
		if(verify_result) {//验证成功
            ...
            self.logger.debug("payment verify succeed");
            //uncomment once the sign works
            //return callback(null,true);
		}
		else {
			//验证失败
            self.logger.debug("payment verify failed");
			self.emit("verify_fail");
            //uncomment once the sign works
            //return callback(null,false);
		}
        //comment it out once the sign works
        return callback(null, trade_status == 'TRADE_SUCCESS');
	});	
}
